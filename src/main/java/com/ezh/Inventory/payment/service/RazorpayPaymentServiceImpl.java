...
import com.ezh.Inventory.payment.repository.RazorpayTransactionRepository;
...
import com.ezh.Inventory.utils.common.dto.IntegrationDto;
import com.ezh.Inventory.utils.common.dto.IntegrationType;
...
public class RazorpayPaymentServiceImpl implements RazorpayPaymentService {
     ...
     * Tries live (RAZORPAY) first, falls back to test mode (RAZORPAY_TEST).
     * <p>
     * IntegrationDto field mapping:
     * <ul>
     *   <li>{@code primaryKey}   — Razorpay Key ID (e.g. {@code rzp_live_xxx})</li>
     *   <li>{@code secondaryKey} — Razorpay Key Secret</li>
     *   <li>{@code tertiaryKey}  — Webhook Secret</li>
     *   <li>{@code extraConfig}  — JSON; may contain {@code "currency"} (default: INR)</li>
     * </ul>
     */
    private IntegrationDto fetchIntegration() throws CommonException {
        try {
            return authServiceClient.getIntegrationByType(IntegrationType.RAZORPAY);
        } catch (Exception e) {
            log.debug("Live Razorpay integration not found, trying test mode: {}", e.getMessage());
            try {
                return authServiceClient.getIntegrationByType(IntegrationType.RAZORPAY_TEST);
            } catch (Exception ex) {
     ...
     * Used in webhook and auto-record flows where no HTTP security context exists.
     * Sends {@code X-Internal-Secret} + {@code X-Tenant-Id} headers so the
     * auth service can authenticate the internal call and scope the response.
     */
    private IntegrationDto fetchIntegrationForTenant(Long tenantId) throws CommonException {
        try {
            return authServiceClient.getIntegrationByTypeForTenant(IntegrationType.RAZORPAY, tenantId);
        } catch (Exception e) {
            log.debug("Live integration not found for tenantId={}, trying test mode", tenantId);
            try {
                return authServiceClient.getIntegrationByTypeForTenant(IntegrationType.RAZORPAY_TEST, tenantId);
            } catch (Exception ex) {
                throw new CommonException(
                        "Razorpay integration not found for tenantId=" + tenantId,
                        HttpStatus.BAD_REQUEST);
            }
        }
    }
    ...
    /**
     * Builds a {@link RazorpayClient} from the tenant's stored credentials.
     * {@code primaryKey} = Key ID, {@code secondaryKey} = Key Secret.
     */
    private RazorpayClient buildClient(IntegrationDto integration) throws CommonException {
        try {
            return new RazorpayClient(integration.getPrimaryKey(), integration.getSecondaryKey());
        } catch (RazorpayException e) {
            throw new CommonException("Failed to initialise Razorpay client: " + e.getMessage(), HttpStatus.BAD_GATEWAY);
        }
    }

    /**
     * Reads {@code "currency"} from the integration's {@code extraConfig} JSON.
     * Falls back to {@code INR} when not set.
     */
    private String extractCurrency(IntegrationDto integration) {
        if (integration.getExtraConfig() == null || integration.getExtraConfig().isBlank()) {
    ...
    public RazorpayOrderResponseDto createOrder(RazorpayOrderRequestDto request) throws CommonException {
        ...
        IntegrationDto integration = fetchIntegration();
    ...
    private RazorpayOrderResponseDto createStandardOrder(
            RazorpayOrderRequestDto request, RazorpayClient client, IntegrationDto integration) throws CommonException {
        try {
            long amountInPaise = toP(request.getAmount());
            String currency = extractCurrency(integration);
            ...
            return RazorpayOrderResponseDto.builder()
                    ...
                    .razorpayKeyId(integration.getPrimaryKey())
    ...
    private RazorpayOrderResponseDto createPaymentLink(
            RazorpayOrderRequestDto request, RazorpayClient client, IntegrationDto integration) throws CommonException {
        try {
            long amountInPaise = toP(request.getAmount());
            String currency = extractCurrency(integration);
    ...
    private RazorpayOrderResponseDto createQrOrder(
            RazorpayOrderRequestDto request, RazorpayClient client, IntegrationDto integration) throws CommonException {
        try {
            long amountInPaise = toP(request.getAmount());
            String currency = extractCurrency(integration);
            ...
            QrCode qrCode = client.qrCode.create(qrReq);
    ...
    public CommonResponse<?> verifyAndRecordPayment(RazorpayVerifyRequestDto request) throws CommonException {
        ...
        // Verify signature using tenant's key secret
        verifySignature(request.getRazorpayOrderId(), request.getRazorpayPaymentId(),
                request.getRazorpaySignature(), integration.getSecondaryKey());

        if (request.getQrCodeId() != null) {
            closeQrCode(request.getQrCodeId(), client);
        }
        if (request.getPaymentLinkId() != null) {
            cancelPaymentLink(request.getPaymentLinkId(), client);
        }

        // Idempotency check — don't record twice for the same order
    ...
    public CommonResponse<?> checkOrderStatus(String orderId) throws CommonException {
        ...
        try {
            ...
            if (orderId.startsWith("qr_")) {
                QrCode qrCode = client.qrCode.fetch(orderId);
                ...
                PaymentLink link = client.paymentLink.fetch(orderId);
     ...
     * Uses the stored tenantId to fetch the correct integration without a user JWT.
     */
    private void autoRecordIfNeeded(String resourceId, String razorpayPaymentId) {
        transactionRepository.findByRazorpayResourceId(resourceId).ifPresent(txn -> {
            ...
            try {
                ...
                log.error("autoRecordIfNeeded: Failed for resourceId={}: {}", resourceId, e.getMessage());
            }
        });
    }
    ...
    @Override
    @Transactional
    public CommonResponse<?> handleWebhook(String payload, String signature, Long tenantId) throws CommonException {
        // Fetch tenant's integration to get the webhook secret (tertiaryKey)
        IntegrationDto integration = fetchIntegrationForTenant(tenantId);
        verifyWebhookSignature(payload, signature, integration.getTertiaryKey());
    ...
    private void verifyWebhookSignature(String payload, String signature, String webhookSecret) throws CommonException {
        try {
    ...
    /**
     * Handles {@code payment.failed} — marks the transaction as FAILED.
     */
     ...
     * Updates the transaction to PAID on success, logs the error on failure.
     */
    private void recordPaymentFromTransaction(RazorpayTransaction txn, String razorpayPaymentId, String source) {
        try {
            ...
            transactionRepository.save(txn);
            log.info("Payment recorded from transaction: resourceId={}, paymentRecordId={}",
                    txn.getRazorpayResourceId(), result.getId());

        } catch (Exception e) {
            log.error("recordPaymentFromTransaction failed for resourceId={}: {}",
                    txn.getRazorpayResourceId(), e.getMessage());
    ...
    private void closeQrCode(String qrCodeId, RazorpayClient client) {
        try {
    ...
    private void cancelPaymentLink(String paymentLinkId, RazorpayClient client) {
        try {
